...
For every non-Latin language, each symbol stores a Latin-script transliteration alongside the script label. At launch this rides on the existing `synonyms` field (cheapest path — no new schema, search indexes hit it naturally). If semantic synonyms and transliterations need to be separated later (e.g. for filtered admin views), promoting transliteration to a dedicated `{ [iso]: string }` field is a non-breaking addition.

Transliterations are generated by the same AI seed pass that produces the script label — adding the second string at seed time is effectively free; retrofitting later across 52k symbols is not.
...
- **Symbol seed pipeline writes a Latin transliteration** for every non-Latin language, stored in the `synonyms` field at launch. Search indexes pick it up without additional configuration.
