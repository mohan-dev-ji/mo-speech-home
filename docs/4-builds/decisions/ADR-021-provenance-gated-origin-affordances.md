...
1. **A freshly seeded account shows no origin affordances anywhere** — all its content is library-installed, so every tile, list, item, sentence, and phrase collapses to `'origin'`/`'none'`. The admin's authoring account is unaffected: its source rows have no `librarySourceId`, so multilingual defaults can still be authored and translated exactly as ADR-019/020 describe.
