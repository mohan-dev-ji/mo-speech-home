...
Single shared service account JSON for **TTS + Imagen** in project `mo-speech-prod`. Already provisioned:

- SA: `mo-speech-app@mo-speech-prod.iam.gserviceaccount.com`
- Roles: `Vertex AI User`, `Service Usage Consumer`
- Local path: `~/.gcloud/mo-speech-app.json` (chmod 600)
- Region for Imagen calls: `us-central1` (Imagen 4 Fast is not available in EU regions yet)

Env vars (added during the TTS swap step of the migration):
- `GOOGLE_SERVICE_ACCOUNT_JSON=<inline JSON>` — used by both `/api/tts` and `/api/ai-generate/imagen`
- `GOOGLE_CLOUD_PROJECT_ID=mo-speech-prod` — added when Imagen route is built
- `GOOGLE_CLOUD_LOCATION=us-central1` — added when Imagen route is built
...
|---|---|---|---|---|
| `mo-speech-prod` (new, 2026-04-28) | **Active** | TTS API + Vertex AI API + `mo-speech-app` SA | TTS (post-swap), Imagen (when built) | **Target home** for all Google APIs |
...
| `mo-speech-465721` (Nov 2025) | Active | Clerk Google sign-in OAuth callback | Clerk only — no app code | Leave (OAuth only, no API quota; moving risks breaking inherited MVP sign-ins) |
| `mo-speech-home` (2026-04-27) | **Abandoned** | Was the original CSE target; CSE turned out to be broken at account level | Nothing | Delete after verifying nothing else was provisioned there |
...
**M1. Provision in `mo-speech-prod`** *(done 2026-04-28)*
- [x] Enable APIs: Cloud Text-to-Speech, Vertex AI, Custom Search.
- [x] Billing linked.
- [x] SA `mo-speech-app@mo-speech-prod.iam.gserviceaccount.com` created with `Vertex AI User` + `Service Usage Consumer`.
- [x] JSON key downloaded to `~/.gcloud/mo-speech-app.json` (chmod 600).

**M2. Swap TTS credentials**
- [ ] Add `GOOGLE_SERVICE_ACCOUNT_JSON=<inline JSON of mo-speech-app.json>` to `.env.local`.
- [ ] Update `app/api/tts/route.ts:16` to read `GOOGLE_SERVICE_ACCOUNT_JSON` instead of `GOOGLE_TTS_CREDENTIALS_JSON`.
- [ ] Test TTS locally: trigger synthesis from a symbol with no cached audio, confirm R2 write + `ttsCache` row + audio playback.
- [ ] Once green, remove `GOOGLE_TTS_CREDENTIALS_JSON` line.
- [ ] In Vercel/prod env (when ready), do the same swap.

**M3. Decommission old TTS project**
- [ ] In `rational-terra-426105-u4`: disable the `tts-account@…` service account (don't delete yet — keeps a 7-day rollback option).
- [ ] After 7 days of green prod TTS: delete the SA and shut down the project.
