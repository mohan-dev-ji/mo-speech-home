...
| Figma component | Variants | Code target | Status | Key spec |
|---|---|---|---|---|
| **grid-symbol** `1435:22432` | Symbol / Symbol-search / Symbol-edit | `shared/ui/SymbolCard.tsx` (+ `categories/ui/SymbolCardEditable.tsx` for edit) | 🔁 | base Symbol=no card bg, `Frame64=symbol-bg`, label+icon=`text`(dark, stays legible). **Symbol-edit** & **Symbol-search** = `bg-theme-card` + `rounded` + `Icon-button`s. Search-result needs a save/edit **Icon-button** |
...
| **List-item** `3025:2524` | Default / List-item-edit | `lists/ui/ListItemAtoms.tsx` · `lists/sections/ListDetailDisplay.tsx` | 🔁 | `bg-theme-card`, `rounded-theme-card`. `tag`=banner, text=`alt-text`, symbol label=`text` |
| **List-item-EDIT** `3025:2324` | List-strip / List-strip-edit | `lists/ui/ListItemAtoms.tsx` (strip layout) | 🔁 | horizontal strip row variant of List-item |
