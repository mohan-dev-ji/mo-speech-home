...
Yes put with the other adrs and a looser principle is better to allow us to pivot if needed as these are relatively unknown in the building but known in how the ux/ui should be.  I will describe as much as I can here. 



Starting with themes - including the ones we have. I think we need to establish a set way themes should be expressed - for example I think the bg should dictate the dark or light look of the theme. Then we can have a base palette of greys that are fine tuned by their opacity and bg-blur. Then the primary colour we have atm which is a tailwind 500 token can be more of a highlight or accent.  This leads into us being able to do some really eye catching glassmorphic effects easily as the bg color can be an animation, gradient, tile, image or flat color. Then the cards, bars, modals and other ui structures can be the universal set of greys - or even just white or black where we can adjust the opacity and blur to make the desired effect. These can be made in the admin dashboard with a simple app preview at the top when you actually go into the theme id page.  Keeping the preview sticky at the top and while the actual settings scroll below. This feature is something we can roll out in later versions of Mo Speech if it is requested enough. These themes can be published, edited and saved like the packs and user will be able to load them depending on their plan from the resource library.



The language will work in a similar way but with obvious differences. I am now thinking that the free tier will just have just have 1 language slot, pro can have 2 language slots and max has 3 language slots. We can modify this later on user feedback and demand of other working architectures. I think this is the simplest way keeping the scope as wide as possible while taking into consideration realistic situations.  Important to remember there are 2 levels to every language.  We have the copy of the marketing site and app and we have the 52k symbolstix symbols and all the tts cache of new words and sentences from lists, sentences and custom symbols all getting generated by the symbol editor into their particular voice.  To add an extra layer of complexity - each language will start with 4 voices chosen in the profile modals under the language selection. Male and female in adult and child voices.  This means for me to make a new language I need to translate the 52k symbols in convex, default lists, default sentences and copy for the app and marketing site.  The technicalities is much well explained here:

/Users/mohanveraitch/Projects/mo-speech-home/docs/4-builds/decisions/ADR-009-multi-language-multi-voice-architecture.md
