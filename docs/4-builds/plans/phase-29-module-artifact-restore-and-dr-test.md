...
|---|---|---|---|---|
| `instruments` | Instruments | image search | `app/api/image-search/search` | free |
| `storybook` | Storybook | AI generation | `app/api/ai-generate/imagen` | pro |
| `clothes` | Clothes | upload | `app/api/upload-asset` | max |

**Topics chosen against measured coverage, 2026-08-23.** The first set (`weather` / `jobs` / `my-home`) was dropped: probing all **1,099** symbols in the catalogue showed `weather` duplicated 10 of 12 labels already in `nature`, and `my-home` 9 of 12 already in `home`. A second pass then moved each topic onto the pipeline it actually suits. The final set overlaps nothing:

| Slug | Probe coverage before | Why this pipeline |
|---|---|---|
| `instruments` | **1 / 12** — only "piano", and listed there as an *activity*, not an instrument | A violin, viola and cello are distinguishable in a **photograph** and near-identical as line glyphs. Photographic detail carries meaning a symbol set cannot, and instruments are among the best-photographed objects on Wikimedia/Unsplash. |
| `storybook` | 1 / 11 — only "book" | Dragons and castles **cannot be photographed** and have no glyphs, so generation is the only option rather than a gimmick. |
| `clothes` | **0 / 11** — the catalogue's single largest gap | Clothing vocabulary is inherently personal: a stock photo of "a jumper" does not help a child request *their* red jumper. Photographing the garment is the point, not a workaround — which makes this the clearest demonstration of what the upload tier is for. |

Two earlier candidates were rejected on content grounds, not coverage: **money** (physical-currency recognition is much less load-bearing now that most transactions are digital) and **hair & appearance** (a clean 2/12 gap, but photographs of a stranger's hair make no sense in a shared catalogue — it belongs in a family's own unpublished module).

**Catalogue policy decided with this choice — a published upload module is a TEMPLATE, not someone's photo album.** Published modules land in a library other families install from, so photos of one family's kitchen do not belong there. `clothes` complies because it ships as a clean flat-lay set of ordinary garments on a plain background — useful to any family as a baseline — and the max tier's value is that a family then *replaces individual images* with photos of their own child's actual coat. That is the product story: the module arrives with good generic photos and personalises in place. A module that only makes sense for its author (`my-home`) stays unpublished in that account, which the install-and-edit flow already supports.
...
| `convex/data/categories/{instruments,storybook,clothes}.json` | Created by exporter | The three new custom-imagery modules, once authored and published. |
...
**Files:** none by hand — authored in the app; `convex/data/categories/{instruments,storybook,clothes}.json` appear via the exporter in Step 8.
...
**Before starting:** have the upload source photos ready on disk for `clothes` — real garments shot flat on a plain background, 12–20 of them. Nothing else in this task blocks on external input.
...
- [ ] **Step 2: Build `instruments` — image search, free tier**

In the admin profile, create category **Instruments** and populate 12–20 symbols via the image-search picker (`app/api/image-search/search`). Labels: guitar, drums, piano, keyboard, violin, flute, trumpet, ukulele, tambourine, xylophone, maracas, recorder.

Only "piano" appears anywhere in the existing 1,099 symbols, and there it is an *activity* rather than an instrument — a different sense, so including it here is not duplication.

Prefer images that show the whole instrument against a plain background at a consistent angle; a cropped detail shot of a guitar neck is unrecognisable at tile size.
...
- [ ] **Step 4: Build `clothes` — uploads, max tier**

Create category **Clothes**, uploading 12–20 of your own photographs via `app/api/upload-asset`. Labels: t-shirt, jumper, trousers, shorts, dress, socks, shoes, coat, hat, gloves, pyjamas, scarf.

This fills the catalogue's single largest gap — **0 of 1,099** existing symbols cover clothing, despite dressing being a daily routine.

Shoot each garment flat, face-up, on the same plain background at the same scale, so the published set is visually consistent and reads as one module. Remember what this set is *for*: it ships as a usable generic baseline, and the max tier's value is that a family swaps individual images for photos of their own child's actual coat. Shoot accordingly — ordinary, unfussy garments rather than anything distinctive.
...
Publish via the Publish modal: `instruments` → **free**, `storybook` → **pro**, `clothes` → **max**. None are `isDefault` — they must not auto-install into new accounts.
...
npx convex run contentModules/exportModules:dumpAllModules --no-push '{}' \
  | node -e 'let s="";process.stdin.on("data",d=>s+=d).on("end",()=>{
      ...
      for(const slug of ["instruments","storybook","clothes"]){
...
Expected: `total: 41`; `instruments` free / `storybook` pro / `clothes` max; every module `personal-leaks=0 ✅`; `imageSourceType` mixes of `imageSearch`, `aiGenerated`, `upload` respectively; no `aiPrompt missing`.
...
instruments (image search, free), storybook (AI generated, pro), clothes (upload, max) —
...
- Open `instruments`, `storybook` and `clothes` and confirm every symbol renders from `library_modules/…` — the post-ADR-022 proof that promoted assets survive a full table wipe.
- Confirm the tier gates still bite after the restore: `instruments` free, `storybook` pro, `clothes` max, and none of the three auto-installed.
...
- **Three custom-imagery modules**, the first non-SymbolStix content in the
  ...
  `instruments` (image search, free), `storybook` (AI generated, pro), `clothes`
...
MOS-13: tick the remaining items — "Build 2–3 demo-tier category modules to test the Publish button" is satisfied by Task 8 (`instruments`, `storybook`, `clothes`), and "Add cover images to the modules" should be confirmed against the exported artifact before ticking:
