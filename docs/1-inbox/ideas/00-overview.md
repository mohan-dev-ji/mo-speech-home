...
```
account (primary instructor — Clerk + Stripe)
  ├── studentProfile (the student — a profile, not a Clerk user)
  └── accountMembers (invited adults — each with their own Clerk login)
```

Any collaborator can use modelling mode. All collaborators share the same student profile and state settings.
...
- **One app, permission-layered** — no separate student/instructor routing trees; state flags control visibility
...
Mo Speech School is the second product — built after Home is proven. It serves teachers and classrooms rather than families. The two products are independent purchases but share a student identity layer (`convex-identity`) that allows:

- A student's profile to be linked across Home and School via an invite code
- The student to switch context between their home and school AAC setup
- Instructors and teachers to view each other's profiles (read-only, on request)
...
07-home-school-connection.md          ← shared student identity, context switching, sharing inbox
08-family-members.md                  ← inviting collaborators, account members model
09-child-profile.md                   ← student profile, state flags, language, home profile storage
