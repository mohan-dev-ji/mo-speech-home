...
type Props = {
  ...
  /** Which content namespace to read group copy from ('lists' | 'sentences').
   *  Both expose the same createGroup* keys. */
  namespace: 'lists' | 'sentences';
};

/**
 * Create a List/Sentence group (ADR-014). Single-name modal, styled to match
 * the existing create modals (CreateListModal / CreateCategoryModal) — same
 * input + footer button design-system tokens.
 */
export function CreateGroupModal({ isOpen, onClose, onCreate, namespace }: Props) {
