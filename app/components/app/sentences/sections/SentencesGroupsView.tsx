...
export function SentencesGroupsView() {
  ...
  const folders = useQuery(api.profileFolders.getProfileFolders, { tree: 'sentences' });
  ...
  const { countByFolder, ungroupedCount } = useMemo(() => {
    ...
    for (const s of sentences ?? []) {
      if (s.folderId) map.set(s.folderId, (map.get(s.folderId) ?? 0) + 1);
      else ungrouped++;
    }
  ...
  async function handleCreate(name: string) {
    await createFolder({ tree: 'sentences', name: { en: name } });
  }
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    setLocalOrder((prev) => {
      ...
      reorderFolders({ tree: 'sentences', orderedIds: next as Id<'profileFolders'>[] });
      return next;
    });
  }

  const loading = folders === undefined || sentences === undefined;
              ...
              <div className={`grid gap-3 ${GROUPS_GRID_CLASSES[stateFlags.grid_size ?? 'large']}`}>
                {orderedFolders.map((folder) => (
                  <SortableGroupTile
                    ...
                    language={language}
                    isEditing={isEditing}
                    onOpen={() => router.push(`/${locale}/sentences/folder/${folder._id}`)}
                ...
                {ungroupedCount > 0 && (
                  <button
                    type="button"
                    onClick={() => router.push(`/${locale}/sentences/folder/ungrouped`)}
