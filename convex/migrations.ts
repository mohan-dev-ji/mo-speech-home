 ...
 */
/**
 * Re-key `aiImageCache` rows after the hash recipe changed (MOS-46).
 *
 * `aiImageCacheHashInput` now wraps the prompt in its style template, so every
 * existing row's stored `hash` was computed by the OLD recipe and no lookup
 * will ever find it again. Without this migration the fix silently throws away
 * every cached image; with it, nothing is lost.
 *
 * ONLY ROWS WHOSE `model` MATCHES THE CURRENT CONSTANT ARE RE-KEYED, and that
 * restriction is the whole correctness argument. A row's image was produced by
 * some (model, template) pair. Re-keying it asserts "this image is what the
 * CURRENT pair produces". That holds for rows generated by the current model
 * under a template that has not changed since — and it does NOT hold for the
 * 36 Imagen-era rows, whose images came from a different model entirely. Those
 * are already unreachable (the model is in the key) and stay that way; they
 * are `scripts/sweep-cache-orphans.mjs` material, not this migration's.
 *
 * Measured before running, 2026-08-30: 46 rows, 10 reachable, 36 Imagen-era.
 * All 10 correspond to current templates — the two `photorealistic` ones were
 * generated after MOS-40's template fix, so they are not stale either.
 *
 * Idempotent: a row already carrying its new hash is left alone, so a second
 * run reports 0 changed.
 *
 * Run:
 *   npx convex run migrations:rehashAiImageCache --no-push '{}'
 *   npx convex run migrations:rehashAiImageCache --no-push '{"apply":true}'
 */
export const rehashAiImageCache = internalMutation({
  args: { apply: v.optional(v.boolean()) },
  handler: async (ctx, { apply }) => {
    const rows = await ctx.db.query("aiImageCache").collect();

    let changed = 0;
    let alreadyCurrent = 0;
    let skippedOtherModel = 0;
    const samples: string[] = [];

    for (const row of rows) {
      if (row.model !== AI_IMAGE_MODEL) {
        skippedOtherModel++;
        continue;
      }
      const input = aiImageCacheHashInput(row.style, row.prompt);
      const digest = await crypto.subtle.digest(
        "SHA-256",
        new TextEncoder().encode(input)
      );
      const nextHash = [...new Uint8Array(digest)]
      ...
      changed++;
      if (samples.length < 12) samples.push(`${row.style}:${row.prompt}`);
      if (apply === true) await ctx.db.patch(row._id, { hash: nextHash });
    }
    ...
    console.log(`[rehashAiImageCache] ${JSON.stringify(summary)}`);
    return summary;
  },
});
