...
export const createProfileList = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // sentences-tree folder) matches no list-folder view and isn't counted as
    // unfiled either, so the list would disappear from every screen with no
    // error. Same check/error shape as moveListToGroup in profileFolders.ts.
    if (args.folderId) {
